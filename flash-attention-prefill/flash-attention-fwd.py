...
def _attn_fwd(
    ...
    # v_order: tl.constexpr = (0, 1) if transV else (1, 0)
...
class _attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, sm_scale, transV):
        # shape constraints
        Lv = v.shape[-2] if transV else v.shape[-1]
...
@pytest.mark.parametrize('Z, H, N_CTX, D_HEAD, dtype, transV',
[ (*shape, dtype, transV)
    ...
    for transV in [False, True]
    ])
def test_op_fwd(Z, H, N_CTX, D_HEAD, dtype, transV):
    ...
    tri_out = attention(q, k, v, sm_scale, transV)
    # compare
...
def bench_flash_attention(BATCH, H, N_CTX, D_HEAD, causal, provider, dtype, device="cuda", transV=False):
    ...
    q = q.to(name_to_torch_types[dtype])
    k = k.to(name_to_torch_types[dtype])
    fn = lambda: attention(q, k, v, sm_scale, transV)
...
def parse_args():
    parser = argparse.ArgumentParser(
        prog="Triton fused flash attention prefill kernel with forward only",
        allow_abbrev=False,
    )
    parser.add_argument("--transV", action='store_true', default=False, help="pre-transpose V tensor")
...
def main():
    args = parse_args()
    bench_flash_attention.run(save_path='.', print_data=True, transV=args.transV)
